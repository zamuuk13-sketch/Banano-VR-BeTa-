...
class TrackingTransport(private val context: Context) {
    ...
    fun autoDiscover(): String? {
        return try {
            ...
            s.send(DatagramPacket(msg,msg.size,address,DISCOVERY_PORT)); s.close(); null
    ...
    fun close(){running.set(false);discoveryThread?.interrupt();discoveryThread=null;socket?.close();socket=null;host=null}
}
